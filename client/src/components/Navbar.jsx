...
<p className='text-sm font-semibold'>{user?.firstName ?? user?.name}</p>
        ...
        <BiChevronDown 
         className='h-8 w-8 text-slate-600' aria-hidden='true'/>
      </Menu.Button>
    </div>
    <Transition
    as={Fragment}
    ...
    >
      <Menu.Item className='absolute z-50 right-2 mt-2
      w-56 origin-top-right divid-y dividfe-gray-100
...
const Navbar = () => {
  ...
  return (
    ...
    <div className='relative bg-[#f7fdfd] z-50'>
      ...
      <div
       className={`${
        isOpen ? "absolute flex bg-[#f7fdfd]" :
        "hidden"
       } container mx-auto lg:hidden flex-col pl-8 
       gap-3 py-5`}
      >
        <Link typeof='/' onClick={handleCloseNavbar}>
        Find Job
        </Link>
        <Link to='/companies' onClick={handleCloseNavbar}>
        Companies
        </Link>
        <Link onClick={handleCloseNavbar}
        to={
        ...
        <div className='w-full py-10'>
          ...
          <div>
            <MenuList user={user} onClick={handleCloseNavbar}/>         </div>
          )}
